...
// Combine both the first and last names and
// populate a new array called `fullNames`. This array will contain just strings.
let fullNames = []; //runners.map(({first_name, last_name}) => `${first_name} ${last_name}`);
...
let firstNamesAllCaps = runners.map(({ first_name }) =>
...
// Problem 1
// Filter big donators with large shirts,
// big donators donate > 100
// tally up donations from those individuals
const largeDonatorsTally = runners
  .filter(({ donation }) => donation > 100)
  .filter(({ shirt_size }) => shirt_size === "L")
  .reduce((tally, { donation }) => tally + donation, 0);

console.log(largeDonatorsTally)

// Problem 2
// find runners with .edu in their email, in order to match their donation etc
const eduRunners = runners.filter(({email}) => email.match(/\.edu$/))
